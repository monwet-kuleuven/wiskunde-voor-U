...
\todo{ook verwijzen naar onafhankelijke/afhankelijke variabele...?}
...
\node[fctie,font=\Large\bfseries, below of= functief, node distance=2cm, text width=13em]     (functiek)  {Functie $f(x)=x^2$};
\node [inpt, left=of functiek] (inputk)  {$5$};
\node [inpt,right=of functiek,text width=8em] (outputk) {$f(5)=5^2+3=28$};  
\draw[>->]  (inputk) -- (functiek) -- (outputk);

\node[fctie,font=\Large\bfseries, below of= functiek, node distance=2cm, text width=13em]     (functieka)  {Functie $f(x)=x^2$};
\node [inpt, left=of functieka] (inputka)  {$a$};
\node [inpt,right=of functieka,text width=6em] (outputka) {$f(a)=a^2+3$};  
\draw[>->]  (inputka) -- (functieka) -- (outputka);
...
\begin{remark} \ 
 ...
 \todo{uitleg waarom dat geen functies zijn}
...
\begin{example} (\textit{reële} functies) %, dus van en naar (deelverzamelingen van) de reële getallen $\R$)
\\
...
% Dus: als we spreken over de functie $1/x$, dan bedoelen we meestal onderstaande functie $g$
